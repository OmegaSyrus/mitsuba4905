...
public:
	...
	MicrofacetDistribution(EType type, Float alphaU, Float alphaV, Texture *tex, Float inv_sigmar2, Float inv_sigmah2, bool sampleVisible = true)
		: m_type(type), m_alphaU(alphaU), m_alphaV(alphaV), m_sampleVisible(sampleVisible),
		m_exponentU(0.0f), m_exponentV(0.0f), m_normalMap(tex), m_inv_sigmar2(inv_sigmar2), m_inv_sigmah2(inv_sigmah2)
	{
		m_alphaU = std::max(m_alphaU, (Float) 1e-4f);
		m_alphaV = std::max(m_alphaV, (Float) 1e-4f);
		if (m_type == EPhong)
...
protected:
	EType m_type;
	Float m_alphaU, m_alphaV, m_inv_sigmar2, m_inv_sigmah2;
